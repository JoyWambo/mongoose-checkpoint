...
A simple project demonstrating database operations using Mongoose.
...
2. Set up environment: Create `.env` file with `MONGO_URI=your_mongodb_atlas_uri_here`
3. Start the server: `npm start` or `npm run dev`

## Usage

The server runs on port 3000. Functions are available in `controllers/personController.js`.
